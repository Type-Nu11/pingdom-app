...
const TARGET_OPERATIONS = new Map([
  ['/places/local-hot', ['get']],
  ['/places/trends', ['get']],
]);

const source = process.argv[2];
const sourceLocation = process.argv[3] ?? source;
const outputPath = new URL('../docs/api/map-home-feeds.openapi.json', import.meta.url);

if (!source) {
  throw new Error('Usage: node scripts/sync-map-home-feeds-openapi.mjs <OpenAPI URL or JSON file>');
}
...
const scopedDocument = {
  ...
  info: {
    ...document.info,
    title: `${document.info?.title ?? 'PingDom OpenAPI'} - map home feeds contract`,
  },
  'x-source': {
