...
const outputPath = new URL('../docs/api/reservation-payment.openapi.json', import.meta.url);

// The upstream document auto-generates opaque operationIds (`list`, `create`,
// `get_2`, `list_4`). They are remapped to stable names here so the generated
// contract, `reservationApi`, and `paymentApi` stay readable; the raw ids are
// still asserted so an upstream renumber fails loudly instead of silently
// pointing at the wrong operation.
const TARGET_OPERATIONS = new Map([
  ['/reservations', [
    ['get', 'list', 'listMyReservations'],
...
const scopedDocument = {
  ...
  info: {
    ...document.info,
    title: `${document.info?.title ?? 'PingDom OpenAPI'} - reservation and tourist payment contract`,
  },
  'x-source': {
...
console.log(
  `Wrote ${TARGET_OPERATIONS.size} reservation/payment paths, ${schemaNames.size} referenced ` +
